...
class Game:
    def __init__(self, env: Union[gym.Env, WorldModelEnv], keymap_name: str, size: Tuple[int, int], fps: int, verbose: bool, record_mode: bool, num_eval_episodes: int) -> None:
        self.env = env
    ...
    def run(self, max_time: Optional[float] = None, num_given_steps: Optional[int] = None, max_steps: Optional[float] = None) -> None:
        pygame.init()
        ...
        def draw_game(image):
            ...
            pygame_image = np.array(image.resize((self.width, self.height), resample=Image.NEAREST)).transpose((1, 0))
        ...
        while not should_stop:
            ...
            if do_reset or done or (max_time is not None and info['time'] >= max_time) or (max_steps is not None and info['timestep'] >= max_steps):
                self.episodes += 1
                if num_given_steps is not None:
                    info['num_given_steps'] = num_given_steps
