...
class MessageType(Enum):
    ...
    CLOSE = 4
    GET_GRAY_OBS = 5
    GET_GRAY_OBS_RETURN = 6
    GET_RGB_OBS = 7
    GET_RGB_OBS_RETURN = 8
...
def child_env(child_id: int, env_fn: Callable, child_conn: Connection) -> None:
    ...
    while True:
        ...
        if message_type == MessageType.RESET:
            ...
            child_conn.close()
            return
        elif message_type == MessageType.GET_GRAY_OBS:
            obs = env._env._pool_and_resize()
            child_conn.send(Message(MessageType.GET_GRAY_OBS_RETURN, obs))
        elif message_type == MessageType.GET_RGB_OBS:
            obs = env._env.environment.ale.getScreenRGB2()
            child_conn.send(Message(MessageType.GET_RGB_OBS_RETURN, obs))
        else:
...
class MultiProcessEnv(DoneTrackerEnv):
    def __init__(
        ...
        env_fn: Callable, 
        num_envs: int, 
        should_wait_num_envs_ratio: float = 1.0,
    ) -> None:
    ...
    def get_gray_scale_observation(self) -> np.ndarray:
        for parent_conn in self.parent_conns:
            parent_conn.send(Message(MessageType.GET_GRAY_OBS))
        content = self._receive(check_type=MessageType.GET_GRAY_OBS_RETURN)
        return np.stack(content)

    def get_rgb_observation(self) -> np.ndarray:
        for parent_conn in self.parent_conns:
            parent_conn.send(Message(MessageType.GET_RGB_OBS))
        content = self._receive(check_type=MessageType.GET_RGB_OBS_RETURN)
        return np.stack(content)
