...
class WorldModel(nn.Module):
    ...
    def compute_loss(self, real_batch: Batch, tokenizer: Tokenizer, train_critic: bool = False, training: bool = True, imagine_batch: Optional[Batch] = None, **kwargs: Any) -> Tuple[LossWithIntermediateLosses, dict]:
    ...
    def imagine(self, batch: Batch, tokenizer: Tokenizer, horizon: int, show_pbar: bool = False) -> Tuple[ImagineOutput, dict]:
        ...
        result = ImagineOutput(
            observations=torch.stack(all_observations, dim=1).cpu(),
            actions=torch.cat(all_actions, dim=1).cpu(),
            rewards=torch.cat(all_rewards, dim=1).cpu(),
            ends=ends.cpu(),
            mask_padding=mask_padding_.cpu(),
            envs=batch['envs'].cpu(),
        )

        wm_env_logs = {
            f'info/{str(self)}/reward_imagine': result.rewards.mean().item(),
            f'info/{str(self)}/done_imagine': result.ends.float().mean().item(),
            f'info/{str(self)}/epsilon': wm_env.epsilon,
            f'info/{str(self)}/num_given_block': num_given_block,
        }

        return result, wm_env_logs