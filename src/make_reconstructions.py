...
@torch.no_grad()
def make_reconstructions_from_batch(batch, save_dir, epoch, tokenizer, enable_check=True):
    if enable_check:
        check_batch(batch)

    original_frames = tensor_to_np_frames(rearrange(batch['observations'], 'b t c h w -> b t h w c'))
    rec_frames = generate_reconstructions_with_tokenizer(batch, tokenizer, enable_check=enable_check)
...
@torch.no_grad()
def make_reconstructions_of_trajectories(batch, save_dir, epoch, tokenizer, world_model, enable_check=False, separator_width=2):
    b, t = batch['observations'].shape[:2]
    original_frames = tensor_to_np_frames(rearrange(batch['observations'], 'b t c h w -> b t h w c'))
    original_concate_frames = insert_separators(original_frames, separator_width=separator_width, separator_color=255)
    tokenizer_rec_frames = generate_reconstructions_with_tokenizer(batch, tokenizer, enable_check=enable_check)
    tokenizer_concate_rec_frames = insert_separators(tokenizer_rec_frames, separator_width=separator_width, separator_color=255)
    ...
    outputs = world_model(tokens, batch['envs'])
    ...
    def get_concate_rec_frames_from_outputs(outputs):
        ...
        tokens_observations = torch.cat((obs_tokens[:, 0, 0].unsqueeze(1), tokens_observations), dim=1)  # (B, t*h*w)
        embeddings_observations = tokenizer.embedding(tokens_observations.flatten())  # (B*t*h*w, E)
        e = embeddings_observations.size(1)
        z_q = rearrange(embeddings_observations, '(b t h w) e -> (b t) e h w', b=b, t=t, e=e, h=h, w=w).contiguous()
        output_frames = torch.clamp(tokenizer.decode(z_q, should_postprocess=True), 0, 1)
        output_frames = rearrange(output_frames, '(b t) c h w -> b t h w c', b=b, t=t)
        rec_frames = tensor_to_np_frames(output_frames)
        return insert_separators(rec_frames, separator_width=separator_width, separator_color=255)
    
    teacher_forcing_regression_concate_rec_frames = get_concate_rec_frames_from_outputs(outputs)
    
    # auto-regression
    num_given_blocks = 4
    given_blocks_tokens = tokens[:, :num_given_blocks*(h*w+1)]
    for step in tqdm(range(num_given_blocks*(h*w+1), t*(h*w+1)), disable=True, desc='auto-regression', file=sys.stdout):
        if (step+1) % (h*w+1) == 0:
            given_blocks_tokens = torch.cat((given_blocks_tokens, act_tokens[:, step // (h*w+1)]), dim=1)
        else:
            outputs = world_model(given_blocks_tokens, batch['envs'])
            logits_observations = outputs.logits_observations[:, -1]
            tokens_observations = logits_observations.argmax(dim=-1)
            given_blocks_tokens = torch.cat((given_blocks_tokens, tokens_observations.unsqueeze(1)), dim=1)

    outputs = world_model(given_blocks_tokens, batch['envs'])
    auto_regression_concate_rec_frames = get_concate_rec_frames_from_outputs(outputs)
    
    # save
    separator = np.full((b, separator_width, *original_concate_frames.shape[2:]), 255, dtype=original_concate_frames.dtype)
    res = np.concatenate(
        (original_concate_frames, separator, 
         ...
         auto_regression_concate_rec_frames), axis=1)
    res = np.squeeze(res, axis=-1)  # due to gray scale
    
    for i, image in enumerate(res):
        img = Image.fromarray(image)
        img.save(save_dir / f'{str(world_model)}_epoch_{epoch:03d}_t_{i:03d}.png')
...
@torch.no_grad()
def generate_reconstructions_with_tokenizer(batch, tokenizer, enable_check: bool = True):
    if enable_check:
        check_batch(batch)
