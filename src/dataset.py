...
class AtariTrajInMemory(AtariTrajectory):
    def __init__(
        self,
        data_dir, 
        csv_dir, 
    ...
    def __getitem__(self, idx):
        ...
        return {
            'observations': pad(
                observations.to(torch.float32).unsqueeze(1)/255.0
            ),  # (L, 1, 84, 84), dtype: float32
            'actions': pad(
...
if __name__ == "__main__":
    ...
    dataset = AtariTrajInMemory(
        'dataset/finetune/expert/trajectory/data', 
        'dataset/finetune/expert/segment/csv',
        envs=['YarsRevenge']
    )
